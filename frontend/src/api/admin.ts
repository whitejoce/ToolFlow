import { apiBaseURL, http } from '../lib/http'
import type {
  CreateToolReq,
  CreateToolResp,
...
export async function getExecutors(params?: { targetEnv?: string }) {
  const { data } = await http.get<GetExecutorsResp>('/api/admin/executors', {
    params: {
      target_env: params?.targetEnv || undefined,
    },
  })
...
export function getExecutorsStreamUrl(params?: { targetEnv?: string }) {
  const url = new URL('/api/admin/executors/stream', apiBaseURL)
  if (params?.targetEnv) {
